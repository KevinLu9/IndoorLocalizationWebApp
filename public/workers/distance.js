...
class BluetoothBeacon {
  constructor(id, txPower, x, y, z) {
    ...
    this.kalman = new KalmanFilter(0.5, 0.5, 0.05); //(0.6, 0.5, 0.02);  //(0.5, 0.7, 0.2); //(0.5, 0.7, 0.4); // 0.5, 0.5, 0.05
  }
  addData(time, rssi) {
...
onmessage = ({ data }) => {
  ...
  if (data.command == "rssi") {
    ...
    postMessage({is_print:true, vals: JSON.stringify({"time": bluetoothDataDict[data.id].time, "rssi":  bluetoothDataDict[data.id].rssi, "kalmanRSSI": bluetoothDataDict[data.id].kalmanRSSI, "distance":  bluetoothDataDict[data.id].distance, "kalmanDistance":  bluetoothDataDict[data.id].kalmanDistance})});
